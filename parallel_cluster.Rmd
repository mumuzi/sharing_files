...
- **run R scripts on cluster**
    - basic steps:
        1. connect to the cluster with your ZCU account
            - command: 
                ...
                - `ssh crunch**` # use another less busy node (up to 16 jobs)
                  - to connect to other nodes, just ssh <nodename> from the head node
                  - all your cluster data is available on all the nodes
            - ZCU wiki:
        ...
        8. directly kill a session
            - keyboard command: `screen -S [screen name] -X quit`
    
    - command lines in *Terminal*: an example
    
    ```{r eval=FALSE}
        $ ssh your_account@zoology.ubc.ca   # connect to the main Zoology server
        Password: # type your password
        your_account% ssh cluster    # ssh to the head-node
        ~ $ ls
...
- **parallel computing in R**
    ...
    - a simple example
        ...
        - simulate 100 datasets from normal distribution
